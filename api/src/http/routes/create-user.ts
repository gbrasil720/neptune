...
export async function createUser(app: FastifyInstance) {
  app.post('/users', async(request, reply) => {
    ...
    if(!details.success || !details.data) {
      return reply.status(400).send({ error: 'The user details are not fully completed or are invalid' })
    }
    ...
    if(userExists) {
      return reply.status(400).send({ error: 'The provided email is already registered' })
    }
