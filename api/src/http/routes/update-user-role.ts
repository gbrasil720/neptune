...
export async function updateUserRole(app: FastifyInstance) {
  app.patch('/users/:userId', async(request, reply) => {
    ...
    if(!role.success || !role.data) {
      return reply.status(400).send({ error: 'No valid role was found at the request body' })
    }

    if(user?.role === role.data.role) {
      return reply.status(400).send({ error: `The user that you are trying to set the role ${role.data.role} already has this role` })
    }
